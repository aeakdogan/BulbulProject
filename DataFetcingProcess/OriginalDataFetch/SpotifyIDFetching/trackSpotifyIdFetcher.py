...
def get_checked(url):
    ...
    if response.status_code in error_codes:
        ...
        notify_slack('(1)[ERROR] We got an 404 error!!')
    else:
        return response
...
def fetch_spotify_uris_from_list(track_info_list, output_file):
    ...
    for line in lines:
        ...
        if cnt % 1000 == 0:
            notify_slack('(1) Skipped Line : {}'.format(skipped_line))
            notify_slack('(1) Spotify Track ID Matcher has matched {} LastFM ids'.format(cnt))

notify_slack('Spotify Track ID Matcher started(1)')
atexit.register(notify_slack, message='Spotify Track ID Matcher finished(1)')

file_name = 'unique_tracks.txt'
output_file = 'matched_lastfm_spotify_ids_1.txt'
